...
static int32_t isDir(const char *path) {
  struct stat s;
  int32_t res = stat(path, &s);
  return (res == FS_ESUCCESS) && S_ISDIR(s.st_mode);
}


static int32_t isSeparator(uint8_t chr) {
  return (chr == '/' || chr == '\\');
}


static int32_t makeDirs(const char *path) {
  int32_t err = FS_ESUCCESS;
...
static int32_t checkFilename(const char *filename) {
...
static int32_t fileInfo(
  const char *filename, uint32_t *mtime, size_t *size, bool *isdir
) {
...
bool fs_isDir(const char *filename) {
  bool res;
  int32_t err = fileInfo(filename, NULL, NULL, &res);
  if (err) return false;
  return res;
}
...
static int32_t appendFileListNode(fs_FileListNode **list, const char *path) {
...
static int32_t writeUsingMode(
  const char *filename, const char *mode, const void *data, int32_t size
) {
  ...
  int32_t res = fwrite(data, size, 1, fp);
  fclose(fp);
